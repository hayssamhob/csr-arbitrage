...
export function AlignmentPanel({
  ...
  const availableSizes = useMemo(() => 
    quotes.filter(q => q.valid).map(q => q.amountInUSDT).sort((a, b) => a - b),
    [quotes]
  );
  ...
  // Price deviation
  const deviationPercent = cexPrice > 0 ? ((dexPrice - cexPrice) / cexPrice) * 100 : 0;
  ...
  const direction: AlignmentDirection = 
    bandStatus === "neutral" ? "none" :
    deviationPercent > 0 ? "sell_dex" : "buy_dex";
  ...
  const totalCostBps = dexFeeBps + slippageBps + (gasUsdt / suggestedSizeUsdt) * 10000;
  ...
  return (
    <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={`text-xs px-2 py-0.5 rounded border ${getBandBgColor(bandStatus)}`}>
      ...
      <div className="mb-4 p-3 rounded bg-slate-900/50">
        <div className="flex justify-between items-center">
          ...
          <span className={`font-mono text-lg font-bold ${getBandColor(bandStatus)}`}>
            {deviationPercent > 0 ? "+" : ""}{deviationPercent.toFixed(2)}%
            <span className="text-xs ml-1">({deviationBps.toFixed(0)} bps)</span>
          </span>
        </div>
        ...
        <div className="mt-2 relative h-2 bg-slate-700 rounded overflow-hidden">
          ...
          <div 
            ...
            style={{ 
              ...
              width: `${(config.bands.softPercent - config.bands.neutralPercent) * 10}%` 
            }} 
          />
          <div 
            className="absolute h-full bg-yellow-500/30" 
          ...
          <div 
            ...
            style={{ 
              left: `${Math.min(Math.max(50 + deviationPercent * 10, 0), 100)}%`,
        ...
        <div className={`p-3 rounded border ${getBandBgColor(bandStatus)}`}>
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div>
              ...
              <span className="ml-1 font-mono">${suggestedSizeUsdt.toFixed(0)}</span>
            </div>
            <div>
              <span className="text-slate-500">Tokens:</span>
              <span className="ml-1 font-mono">{suggestedSizeTokens.toFixed(2)}</span>
            </div>
            <div>
              <span className="text-slate-500">Est. Cost:</span>
              <span className="ml-1 font-mono">{totalCostBps.toFixed(0)} bps</span>
            </div>
            <div>
              <span className="text-slate-500">Net Benefit:</span>
              <span className={`ml-1 font-mono ${netBenefitBps >= 0 ? "text-emerald-400" : "text-red-400"}`}>
          ...
          {!isEconomical && (
            <div className="mt-2 text-xs text-yellow-400">
              ⚠️ Trade may not be economical (net benefit &lt; {config.minBenefitBps} bps)
            </div>
          )}
        </div>
      )}
      ...
      <div className="mt-3 flex items-center justify-between text-xs">
        <span className="text-slate-500">Mode: {config.mode.toUpperCase()}</span>
