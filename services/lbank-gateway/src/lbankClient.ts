...
export class LBankClient extends EventEmitter {
  ...
  connect(): void {
    if (this.isConnecting || this.isConnected) {
      this.onLog('warn', 'connect_skipped', { reason: 'already connecting or connected' });
      return;
    }

    this.isConnecting = true;
    this.onLog('info', 'connecting', { url: this.wsUrl, symbols: this.symbols });
  ...
  private handleMessage(data: WebSocket.RawData): void {
    ...
    try {
      ...
      this.onLog('warn', 'parse_error', { error: String(err), raw: data.toString().slice(0, 200) });
      return;
    }
    ...
    if (tickerResult.success) {
      ...
      this.onLog('debug', 'ticker_received', { symbol: normalized.symbol, last: normalized.last });
    ...
    if (depthResult.success) {
      ...
      this.onLog('debug', 'depth_received', { symbol: normalized.symbol, bids: normalized.bids.length });
  ...
  private normalizeTickerData(
    raw: { tick: { latest: string; high: string; low: string; vol: string; change: string }; pair: string; TS?: string },
    receiveTs: string
  ): LBankTickerEvent {
  ...
  private normalizeDepthData(
    raw: { depth: { bids: [string, string][]; asks: [string, string][] }; pair: string; TS?: string },
    receiveTs: string
  ): LBankDepthEvent {
    return {
      type: 'lbank.depth',
      symbol: raw.pair.toLowerCase(),
      ts: receiveTs,
      bids: raw.depth.bids.map(([price, qty]) => [parseFloat(price), parseFloat(qty)]),
      asks: raw.depth.asks.map(([price, qty]) => [parseFloat(price), parseFloat(qty)]),
      source_ts: raw.TS ? new Date(parseInt(raw.TS)).toISOString() : undefined,
    };
  }
  ...
  private scheduleReconnect(): void {
    ...
    const baseDelay = this.reconnectIntervalMs * Math.pow(1.5, this.reconnectAttempts - 1);
